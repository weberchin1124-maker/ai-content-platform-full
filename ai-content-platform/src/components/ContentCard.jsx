      ...
      <div style={{ marginBottom: '15px', flexGrow: 1 }}>
        {isEditing ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
             ...
             {/* 只有當原本就有 response 時，才顯示編輯 response 的框 */}
             {response && (
               <>
      ...
      {isEditing ? (
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 'auto', gap: '15px', alignItems: 'center' }}>
          {onRefine && (
            <button 
             style={{ 
               background: 'none', border: 'none', color: '#a371f7', cursor: 'pointer', 
               fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '6px', 
               padding: '5px 10px', borderRadius: '6px', transition: 'background-color 0.2s' 
             }}
