...
class Project(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=datetime.utcnow)  # 新增：建立時間
...
class Content(db.Model):
    ...
    # 直接在 Content 表存 Prompt 和 Response
    original_prompt = db.Column(db.Text) 
    generated_content = db.Column(db.Text) 

    # 新增 latest_version_id（nullable）以供 routes 寫入
...
class ContentVersion(db.Model):
    ...
    response = db.Column(db.Text) # 已有 response

    # 新增 file_url 與 response_ref（routes 與 service 使用到）
    file_url = db.Column(db.String(512), nullable=True)
    response_ref = db.Column(db.String(255), nullable=True)
