...
def get_project_models():
    """延遲導入 Project, ProjectMember, Content 模型，避免循環引用問題。"""
    from app.models import Project, ProjectMember, Content 
...
def get_my_projects():
    try:
        ...
        return jsonify({"error": "取得專案列表失敗"}), 500


# ==========================================
# 2. 建立專案 (POST /api/projects)
# ==========================================
...
def create_project():
    try:
        ...
        db.session.flush() # 取得 project_id
        
        # 2. 建立 ProjectMember 紀錄 (建立者為 Owner)
...
def delete_project(project_id):
    try:
        ...
        # 1. 刪除所有相關的 Content (確保資料刪乾淨)
        Content.query.filter_by(project_id=project_id).delete(synchronize_session='fetch')

        # 2. 刪除專案本身 (SQL Cascade 會自動刪除 ProjectMember)
...
def get_project_summary(project_id):
    try:
        ...
        for c in contents:
            ...
            if c.latest_version and c.latest_version.prompt:
                notes_data.append({
                    "date": c.created_at.strftime("%Y-%m-%d"),
                    "content": c.latest_version.prompt # 使用最新版 prompt
                })
